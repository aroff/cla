...
use ndarray::array;
use rustcla::{Frontier, FrontierPoint, TurningPoint, TOL_WEIGHTS};
...
fn test_frontier_point_new() {
    ...
    assert!(
        result.is_ok(),
        "FrontierPoint should be created successfully"
    );
}
...
fn test_frontier_point_weights_sum() {
    ...
    assert!(
        result.is_err(),
        "FrontierPoint should reject weights that don't sum to 1.0"
    );
}
...
fn test_frontier_point_mean() {
    ...
    assert!(
        (expected_return - expected).abs() < 1e-10,
        "Expected return should be 0.14"
    );
}
...
fn test_turning_point_new() {
    ...
    assert!(
        result.is_ok(),
        "TurningPoint should be created successfully"
    );
}
...
fn test_frontier_interpolate() {
    ...
    assert!(
        ...
        "Interpolated frontier should have more points"
    );
...
fn test_frontier_weights() {
    ...
    assert_eq!(
        ...
        &[2, 2],
        "Weights matrix should be 2x2"
    );
    assert_eq!(
        weights_matrix[(0, 0)],
        1.0,
        "First point, first asset should be 1.0"
    );
    assert_eq!(
        weights_matrix[(1, 1)],
        1.0,
        "Second point, second asset should be 1.0"
    );
}
...
fn test_frontier_returns() {
    ...
    assert!(
        ...
        "First return should be 0.1"
    );
    assert!(
        (returns[1] - 0.2).abs() < 1e-10,
        "Second return should be 0.2"
    );
}
...
fn test_frontier_variance() {
    ...
    assert!(
        (variances[0] - 0.01).abs() < 1e-10,
        "First variance should be 0.01"
    );
    assert!(
        (variances[1] - 0.04).abs() < 1e-10,
        "Second variance should be 0.04"
    );
}
...
fn test_frontier_volatility() {
    ...
    assert!(
        (volatilities[0] - 0.1).abs() < 1e-10,
        "First volatility should be 0.1 (sqrt(0.01))"
    );
    assert!(
        (volatilities[1] - 0.2).abs() < 1e-10,
        "Second volatility should be 0.2 (sqrt(0.04))"
    );
}
...
fn test_frontier_sharpe_ratio() {
    ...
    assert!(
        (sharpe_ratios[0] - 1.0).abs() < 1e-10,
        "First Sharpe ratio should be 1.0"
    );
    assert!(
        (sharpe_ratios[1] - 1.0).abs() < 1e-10,
        "Second Sharpe ratio should be 1.0"
    );
}
